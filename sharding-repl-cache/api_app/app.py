...
async def root():
    ...
    for collection_name in collection_names:
        ...
        try:
            ...
            per_shard = {
                ...
                for shard_name, shard_info in shard_stats.items()
            }
    ...
    shards = None
    replicas_per_shard: dict[str, int] = {}
    replicas_total = 0
    if topology_type == "Sharded":
        ...
        for shard in shards_list.get("shards", []):
            host_string = shard.get("host", "")
            shards[shard["_id"]] = host_string
            members_segment = host_string.split("/", 1)[1] if "/" in host_string else host_string
            members = [member for member in members_segment.split(",") if member]
            count = len(members)
            replicas_per_shard[shard["_id"]] = count
