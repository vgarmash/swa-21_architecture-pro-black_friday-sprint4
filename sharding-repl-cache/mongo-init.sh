...
docker compose exec -T shard-1-replica-1 mongosh --port 27018 <<EOF
    rs.initiate(
        {
            _id: "shard-1",
            members: [
                { _id: 0, host: "shard-1-replica-1:27018" },
                { _id: 1, host: "shard-1-replica-2:27018" },
                { _id: 2, host: "shard-1-replica-3:27018" }
            ]
        }
    );
EOF
...
docker compose exec -T shard-2-replica-1 mongosh --port 27019 <<EOF
    rs.initiate(
        {
            _id: "shard-2",
            members: [
                { _id: 0, host: "shard-2-replica-1:27019" },
                { _id: 1, host: "shard-2-replica-2:27019" },
                { _id: 2, host: "shard-2-replica-3:27019" },
            ]
        }
    );
EOF
...
sh.addShard( "shard-1/shard-1-replica-1:27018");
sh.addShard( "shard-2/shard-2-replica-1:27019");
