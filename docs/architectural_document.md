...
|:-------------:|:----------------------------|--------------------|-------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|    orders     | Поиск истории заказов       | secondary          | 1 - 5 секунд            | <ul><li>Предполагается, что в общем списке заказов не требуется, чтобы все статусы всегда были актуальными.</li><li>Пользователь редко будет обновлять ленту заказов, чтобы получить актуальный статус - скорее, он зайдет в конкретный заказ.</li><li>Рисков некорректной логики работы из-за задержки нет.</li></ul>                     |
|    orders     | Просмотр заказа             | primary            | Нет                     | <ul><li>При просмотре заказа пользователь всегда должен видеть актуальный статус, поскольку от этого зависят его действия в текущий момент.</li><li>Некорректный статус может привести к некорректным действиям пользователя.</li></ul>                                                                                                    |
|   products    | Поиск товаров               | secondary          | более 10 секунд         | <ul><li>Некритично, если в произвольный момент времени пользователь получит неактуальный список товаров, так как это рутинная ситуация, когда этот список меняется даже в ходе заказа.</li></ul>                                                                                                                                           |
|   products    | Просмотр конкретного товара | secondary          | более 10 секунд         | <ul><li>Некритично, если в произвольный момент времени пользователь получит неактуальное описание товара, так как это рутинная ситуация, когда этот список меняется даже в ходе заказа.</li><li>Также, даже если отображаются неактуальные остатки, они в любом случае будут проверены и могут измениться при оформлении заказа.</li></ul> |
|   products    | Получение остатков товара   | primary            | Нет                     | <ul><li>Перед списанием товара обязательно проверить его наличие, иначе может быть оформлен заказ на отсутствующий товар и его отмена вызовет негативный пользовательский опыт.</li><li>Также могут быть ошибки в логике оформления заказа, если состояние остатков будет неактуальным.</li></ul>                                          |
|     carts     | Получение состояния корзины | primary            | Нет                     | <ul><li>Состояние корзины должно быть максимально актуальным, поскольку иначе пользователь может заказать некорректный набор товаров, либо товары, которых более нет в наличии.</li></ul>                                                                                                                                                  |


# Миграция на Cassandra

## Выбор сущностей для миграции

1. **orders**: В периоды высокой активности часто создаются заказы, еще чаще обновляются их статусы, и критически важно, чтобы эти изменения не были утеряны и обработка заказов оставалась доступной. *Переносим в Cassandra.* 
2. **products**: Обновления каталога товаров относительно редки и производятся изнутри компании, так что потеря данных некритична: можно повторно добавлять новые товары. Также неважно, если запись будет некоторое время недоступна, поскольку каталог необязательно обновлять сразу. *Не переносим в Cassandra.*
3. **carts**: В периоды высокой активности корзина часто меняется. Хотя потеря изменений корзин не настолько критична, она может быть крайне негативна с точки зрения пользовательского опыта и привести к снижению выручки компании. Желательно обеспечить высокую доступность на запись. *Переносим в Cassandra.*
...
**Partition key**: `user_id`, так как размещение заказов разных пользователей на разных узлах позволит более равномерно распределить нагрузку и снизить риски расхождения данных между узлами.
**Clustering key**: `created_at`, так как пользователям в первую очередь интересны более свежие заказы.
...
) WITH default_time_to_live = 60 * 60 * 24;
```

**Partition key**: композитный ключ из `user_id` и `session_id`, так как размещение корзин разных пользователей на разных узлах позволит более равномерно распределить нагрузку и снизить риски расхождения данных между узлами, но мы должны учитывать также корзины незарегистрированных пользователей.
**Clustering key**: не требуется, так как каждый пользователь запрашивает только одну корзину.
**Дополнительно**: устанавливаем `default_time_to_live` на сутки, так как корзина по требованиям должна очищаться раз в день в случае неактивности.

## Восстановление целостности

1. **orders**: Выбираем стратегию *read repair*, так как данные часто читаются, но при этом нам важна консистентность при чтении заказов пользователя.
2. **carts**: Подойдет вариант *hinted handoff*, поскольку актуальность при этом восстанавливается не прямо при чтении, но достаточно быстро. Если использовать *anti-entropy repair*, то корзины могут часто быть в неконсистентном состоянии, пока починка не будет запущена. 