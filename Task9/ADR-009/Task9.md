...
|:-----:|:--------------------------------------------|:--------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------|
| UC11  | Покупатель, Каталог, MongoDB Replica Set    | Просмотр каталога товаров             | Покупатель просматривает список товаров. Система может читать данные с secondary реплик с допустимой задержкой для снижения нагрузки на primary. |
| UC12  | Покупатель, Заказы, MongoDB Replica Set     | Проверка статуса активного заказа     | Покупатель проверяет статус недавно оформленного заказа. Система должна читать с primary для обеспечения актуальности данных.                    |
| UC13  | Покупатель, Корзина, MongoDB Replica Set    | Обновление корзины в реальном времени | Покупатель добавляет/удаляет товары. Все операции должны выполняться на primary для мгновенной консистентности.                                  |
| UC14  | Система инвентаризации, MongoDB Replica Set | Валидация остатков при оформлении     | При финализации заказа система проверяет актуальные остатки товаров исключительно с primary узла для предотвращения overselling.                 |
| UC15  | Аналитик, Отчётность, MongoDB Replica Set   | Генерация аналитических отчётов       | Система формирует отчёты по историческим данным, читая с secondary реплик с допустимой задержкой до 30 секунд.                                   |
...
| **№** | **Требование**                                                                                                                                                                                |
|:------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| F1    | Система должна поддерживать различные уровни консистентности данных - strong consistency для критичных операций (остатки, оплата) и eventual consistency для некритичных (просмотр каталога). |
| F2    | Возможность настройки стратегии чтения (primary/secondary/secondaryPreferred) на уровне отдельных операций и коллекций.                                                                       |
| U1    | Пользователи не должны замечать разницу в консистентности данных при нормальной работе системы (задержка репликации <5 сек).                                                                  |
| U2    | Критичные операции (оплата, заказ) всегда должны показывать актуальные данные без исключений.                                                                                                 |
| R1    | При недоступности secondary узлов система должна автоматически переключаться на primary без потери функциональности.                                                                          |
| R2    | Гарантия отсутствия overselling и дублирования заказов при любых условиях работы репликации.                                                                                                  |
| P1    | Минимум 60% read-операций должны обрабатываться secondary репликами, снижая нагрузку на primary на 40-50%.                                                                                    |
| P2    | Lag репликации не должен превышать: 5 сек для products, 10 сек для orders, 60 сек для архивных данных.                                                                                        |
| S1    | Автоматический сбор метрик lag репликации, количества операций на primary/secondary, с алертами при превышении порогов.                                                                       |
| S2    | Возможность изменения maxStalenessSeconds и Read Preference без перезапуска приложения через конфигурационные файлы.                                                                          |
