...
|--|--|--|--|
| Поиск товаров по категории / фильтрация по цене (каталог, поиск) | Secondary | <2c | - Данные (название, цена, категория) редко меняются. \n - Небольшая задержка (≤500 мс) незаметна пользователю. \n — Снижает нагрузку на primary. |
| Просмотр страницы товара (описание, атрибуты) | Secondary | <2c | — Метаинформация стабильна. — Даже при изменении — пользователь не заметит устаревшего описания 10-секундной давности. |
...
| Проверка остатка при оформлении заказа | Primary | 0c | — Часть атомарной операции: проверка → списание. — Требуется линейная консистентность. — Не может читаться с secondary — иначе race condition |
...
| Операция | Режим чтения | Допустимая задержка | Обоснование |
|--|--|--|--|
| Просмотр истории заказов пользователя | Secondary | <2c | Пользователь ожидает увидеть актуальный статус, на задержака до 2 секунд не критична |
